...
\begin{frame}{Why?}
    ...
    \only<3>{
      \begin{figure}  
        ...
        \caption{The 2008-9 Yellostone Lake eq. swarm \cite{Massin:2013ts}.}\label{yles}
      \end{figure} }
...
\begin{frame}{Conventional approach}
  ...
  \only<1>{
    \begin{figure}  
      ...
      \caption{From picks to precise location \cite{Shelly:2013tq,Stamps:2006vb,Waldhauser:2000ur}.}\label{picklocreloc}
    \end{figure} }
  ...
  \only<2>{
    \begin{figure}  
      ...
      \caption{... to moment tensor \cite{Godano:2015ti,Shelly:2013tq}.}\label{mo}
    \end{figure} }
	...
	\only<4>{\textbf{\textit{How to use more data in source parameter analysis ? In a more direct way ?}}}
\end{frame}

\begin{frame}{Shift-\&-stack approach (or source scanning, or SSA)}
  Hypocenter scanning \cite{Zahradnik:2015wf,Frank:2014gl,Langet:2014tq,Kao:2004dj}
	\begin{itemize}
		\item Grid pre-calculation of \alert{\only<1>{travel time}\only<2>{amplitudes}} for \alert{\only<1>{trial source positions}\only<2>{take off angles}},
		\item Signal pre-processing with body-wave \alert{\only<1>{characteristic functions}\only<2>{wavelets}},
		\item stacking using pre-defined \alert{\only<1>{travel times} \only<2>{amplitudes}}:
		\begin{itemize}
			\item probability \alert{\only<1>{grid}\only<2>{sphere}},
			\item \alert{\only<1>{centro\"{i}d}\only<2>{P-T axis}} are defined with bayesian approach.
		\end{itemize}
	\end{itemize}	
  How \textbf{\textit{\only<1>{to explore more spaces?}\only<2>{is this better?}}}
...
\begin{frame}{Examples\only<2->{ improvements with CECM}\only<4->{ and muti-scaling}}
  % a two col frame :
  \begin{columns}[T]
